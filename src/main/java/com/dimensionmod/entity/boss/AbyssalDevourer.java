...
public class AbyssalDevourer extends Monster {
    private static final int PHASE2_HEALTH = (int) (600 * 0.4);
    ...
    private List<net.minecraft.core.BlockPos> spikePositions = new ArrayList<>();
    ...
    private void startSpikeAttack() {
        ...
        for (int x = -4; x <= 4; x++) {
            for (int z = -4; z <= 4; z++) {
                if (Math.abs(x) + Math.abs(z) <= 5 && (x != 0 || z != 0)) {
                    net.minecraft.core.BlockPos pos = this.blockPosition().offset(x, 0, z);
        ...
        for (net.minecraft.core.BlockPos p : spikePositions) {
    ...
    private void summonMinions() {
        for (int i = 0; i < 3; i++) {
            ...
            if (this.level().getBlockState(spawnPos).isAir()) {
                LivingEntity minion = (LivingEntity) EntityType.ZOMBIE.create(this.level());
                if (minion != null) {
                    minion.setPos(spawnPos.getX() + 0.5, spawnPos.getY() + 0.5, spawnPos.getZ() + 0.5);
                    minion.setCustomName(Component.literal("§5深渊侍从"));
    ...
    private void spawnAbyssParticles() {
        for (int i = 0; i < 4; i++) {
            this.level().addParticle(ParticleTypes.AMBIENT_ENTITY_AT,
                    ...
                    this.getZ() + (this.random.nextDouble() - 0.5) * 3, 0, 0.02, 0);
        }
    }
    ...
    public void customServerAiStep() {
        ...
        this.bossInfo.setName(
                phase == 2 ? Component.literal("§5深渊吞噬者 §d【觉醒】") : Component.literal("§5深渊吞噬者"));
    }
